...
void msTablePrint(Table table) {
    ...
    while(it[0] <= KING) {
        ...
        for(size_t j = 0; j < PLAYERS; ++j) {
            printf("%6zu ", table.occurrences[i].sub[j]);
        } putchar(' ');

        for(size_t j = 0; j < LANCES; ++j) {
            for(size_t k = 0; k < PLAYERS; ++k) {
                printf("%6zu ", table.categories[i][j].sub[k].occurr);
            } putchar('|');
        }
        ...
        for(size_t j = 0; j < LANCES; ++j) {
            for(size_t k = 0; k < PLAYERS; ++k) {
                printf("%6zu ", table.categories[i][j].sub[k].amarrW);
            } putchar('|');
        }
        ...
        for(size_t j = 0; j < LANCES; ++j) {
            for(size_t k = 0; k < PLAYERS; ++k) {
                printf("%6zu ", table.categories[i][j].sub[k].amarrL);
            } putchar('|');
        }
